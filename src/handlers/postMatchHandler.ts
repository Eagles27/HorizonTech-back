...
import { TUser, TUserPostMatchBody, USER_POST_MATCH_BODY } from '@/schema/user'
...
const postMatchHandler = async (req: TRequest, res: FastifyReply) => {
  ...
  try {
    ...
    const updatedUser = await USER_MONGOOSE.findOneAndUpdate(
      { _id: userId },
      { $push: { contacts: { contact_id: req.body.contact_id, accepted: true } } },
      { new: true }
    )
    ...
    await USER_MONGOOSE.findOneAndUpdate(
      ...
      { $push: { contacts: { contact_id: userId._id, accepted: false } } },
      { new: true }
    )

    res.code(201).send({ _id: updatedUser._id })
  } catch (error) {
